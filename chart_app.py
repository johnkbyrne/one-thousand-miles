...
def bkapp_page():
    ...
    with pull_session(url="http://localhost:5006/chart") as session:

        # update or customize that session
        session.document.roots[0].children[1].title.text = "Special Sliders For A Specific User!"

        # generate a script to load the customized session
        script_ = server_session(session_id=session.id, url='http://localhost:5006/chart')
        script, div = components(cumulative_actual)
        a_script, a_div = components(summary_actual)
        layout_script, layout_div = components((weeks_runs, weekly_actual_cumulative_fig, p, week_stacked_bar))

        # use the script in the rendered page
        return render_template("chart.html", script=script_, template="Flask",
                                the_div=div,
    							the_script=script,
    							actual_div=a_div,
